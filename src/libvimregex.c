...
int vimre_vmatch(unsigned char *pattern, unsigned char *s,
        OnigRegion *region)
{
    copied_pattern = 0;
    vim_pattern    = pattern;
    ...
    return vimre_omatch(onig_pattern, s, region);
}

int vimre_omatch(unsigned char *pattern, unsigned char *s,
        OnigRegion *region)
{
    int            r;
